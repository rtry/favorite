...
//				+ "        \"source\" : \"" + article.getSource() + "\"," + "        \"url\" : \""
//				+ article.getUrl() + "\"," + "        \"pubdate\" : \""
...
//		JestClient jestClient = JestExample.getJestClient();
//		Suggest suggest = new Suggest.Builder("{" + "  \"" + suggestionName + "\" : {"
//				+ "    \"text\" : \"the amsterdma meetpu\"," + "    \"term\" : {"
//				+ "      \"field\" : \"body\"" + "    }" + "  }" + "}").build();
//		SuggestResult suggestResult = jestClient.execute(suggest);
//		System.out.println(suggestResult.isSucceeded());
//		List<SuggestResult.Suggestion> suggestionList = suggestResult
//				.getSuggestions(suggestionName);
...
//	// Search search = new
//	// Search.Builder(searchSourceBuilder.toString()).addIndex("article")
//	// .build();
//	// SearchResult result = jestClient.execute(search);
...
//				"http://news.163.com/15/0909/14/B332O90E0001124J.html", Calendar.getInstance()
//						.getTime(), "中国青年网", "匿名");
//		Article article2 = new Article(
//				5,
//				"中央党校举行秋季学期开学典礼 刘云山出席讲话",
...
//						+ "赵乐际出席开学典礼。"
//						+ "中央有关部门负责同志，中央党校校委成员、全体学员和教职工在主会场参加开学典礼。中国浦东、井冈山、延安干部学院全体学员和教职工在分会场参加开学典礼。",
//				"http://news.163.com/15/0907/20/B2UGF9860001124J.html", Calendar.getInstance()
//						.getTime(), "新华网", "NN053");
//		Article article3 = new Article(
//				6,
//				"俞正声率中央代表团赴大昭寺看望宗教界人士",
...
//				"http://news.163.com/15/0907/16/B2U3O30R00014JB5.html", Calendar.getInstance()
//						.getTime(), "国际在线", "全宇虹");
//		Article article4 = new Article(
//				7,
//				"张德江:发挥人大主导作用 加快完备法律规范体系",
...
//				"http://news.163.com/15/0907/20/B2UGEUTJ00014PRF.html", Calendar.getInstance()
//						.getTime(), "新华网", "陈菲");
//
...
//				.defaultType("article")
//				.addAction(
//						Arrays.asList(new Index.Builder(article1).build(), new Index.Builder(
//								article2).build(), new Index.Builder(article3).build(),
//								new Index.Builder(article4).build())).build();
//		jestClient.execute(bulk);
//	}
//
//	private static void createIndex() throws Exception {
//		JestClient jestClient = JestExample.getJestClient();
//		Article article1 = new Article(1, "高圆圆身上淤青 遭家暴还是玩SM遭性虐？",
//				"近日，有媒体拍到高圆圆身上的淤青，腿上还有两块伤疤，引起不少人猜测是遭受家暴。"
...
//						+ "事情真的这么简单吗？即使稍微一撞，也不至于淤青吧！" + "看到那个伤疤以及淤青的皮肤，不得不让人怀疑高圆圆是遭受家暴。"
//						+ "当然，还有另外一个原因，就是玩SM遭性虐。" + "当然，这么变态的事情，相信女神不会做的。"
//						+ "是照顾母亲留下的伤痕也好，遭受家暴也好，希望女神高圆圆以后都能平平安安健健康康吧！",
//				"http://www.vdfly.com/star/20141119/37968.html", Calendar.getInstance().getTime(),
//				"青春娱乐网", "匿名");
//		Article article2 = new Article(
//				2,
//				"习近平:希望新西兰保障输华产品质量安全",
//				"新华网惠灵顿11月20日电（记者陈贽 田帆刘华）国家主席习近平20日在惠灵顿同新西兰总理约翰·基举行会谈，双方决定，将中新关系提升为全面战略伙伴关系，共建中新两国利益共同体，推动两国合作不断迈上新台阶。"
...
//						+ "我们愿同中方共同推动两国全面战略伙伴关系发展。王沪宁、栗战书、杨洁篪等参加上述活动。",
//				"http://news.163.com/14/1120/15/ABGM1POL00014JB5.html", Calendar.getInstance()
//						.getTime(), "新华网", "NN053");
//		Article article3 = new Article(
//				3,
//				"周末陕西将迎雨雪天气 关中近日大部雾霾缭绕",
//				"本报讯（首席记者 姬娜）前天，我省47个县城出现雾霾。昨天，关中大部分地区依旧雾霾缭绕，能见度差。预计22日-24日有雨雪天气，届时雾霾或将能得到缓解。对于这样的天气，微信上不再如之前有那么多埋怨牢骚，更多的是无奈。"
...
//						+ "21日：陕北、关中多云，陕南多云转阴天，南部局地有小雨。22日：陕北、关中多云转阴天，陕南阴天，关中南部和陕南大部有小雨。23日：全省阴天，关中部分有小雨，陕南有小到中雨。24日：全省阴天，陕北部分地方有雨夹雪，关中、陕南有小雨。25日：全省阴天转多云。26日：全省多云。后期天气趋势（27-29日）：我省以阴到多云为主，中南部仍有阴雨天气。",
//				"http://xian.qq.com/a/20141120/011300.htm", Calendar.getInstance().getTime(),
//				"三秦都市报 - 三秦网", "姬娜");
//		Index index1 = new Index.Builder(article1).index("article").type("doc").build();
//		Index index2 = new Index.Builder(article2).index("article").type("doc").build();
//		Index index3 = new Index.Builder(article3).index("article").type("doc").build();
...
//		factory.setHttpClientConfig(new HttpClientConfig.Builder("http://192.168.253.104:9200")
//				.gson(new GsonBuilder().setDateFormat("yyyy-MM-dd HH:mm:ss").create())
//				.multiThreaded(true).readTimeout(10000).build());
