...
/**
 * 类名称：CharsetDetector <br>
 * 类描述: 字符集自动检测 <br>
 * 创建人：felicity <br>
 * 创建时间：2018年7月2日 上午11:22:34 <br>
 * 修改人：felicity <br>
 * 修改时间：2018年7月2日 上午11:22:34 <br>
 * 修改备注:
 * @version
 * @see
 */
public class CharsetDetector {
	...
	private static Pattern charsetPatternHTML5 = Pattern.compile(
			"<meta\\s+charset\\s*=\\s*[\"']?([a-z][_\\-0-9a-z]*)[^>]*>", Pattern.CASE_INSENSITIVE);
	...
	public static String guessEncoding(byte[] content) {
		try {
			return guessEncodingByNutch(content);
		} catch (Exception ex) {
	...
	private static String guessEncodingByNutch(byte[] content) {
		...
		if (encoding == null) {
			if (length >= 3 && content[0] == (byte) 0xEF && content[1] == (byte) 0xBB
					&& content[2] == (byte) 0xBF) {
				encoding = "UTF-8";
			} else if (length >= 2) {
